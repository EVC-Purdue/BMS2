...
namespace t_logger
{
    ...
    static const char *TAG = "t_logger";
    ...
    void TLogger::write_buffer_to_spiffs()
    {
        ...
        if (this->spiffs_usage_write_count >= SPIFFS_RECHECK_USAGE_WRITES_COUNT)
        {
            size_t total = 0;
            size_t used = 0;
            esp_err_t fs_info_ret = esp_littlefs_info(nullptr, &total, &used);
            if (fs_info_ret != ESP_OK || total == 0)
            {
                ESP_LOGE(TAG, "LittleFS info unavailable (%s), dropping buffered log write", esp_err_to_name(fs_info_ret));
        ...
        if (file == nullptr)
        {
            ESP_LOGE(TAG, "Failed to open log file at %s", LOG_FILE_PATH);
            this->write_buffer_index = 0;
            return;
        }
