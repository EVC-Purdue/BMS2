...
uint32_t FaultManager::get_persistent_faults() const {
    return this->current_set_faults & persistent_faults_mask;
}

bool FaultManager::get_current_fault(size_t fault_bit) const {
    return (this->current_set_faults & (static_cast<uint32_t>(1) << fault_bit)) != 0;
}
