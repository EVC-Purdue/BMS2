...
namespace faults
{
    ...
    private:
        ...
        const uint32_t persistent_faults_mask = (static_cast<uint32_t>(1) << faults::PersistentFault::PERSISTENT_FAULTS_END) - 1;
        const uint32_t live_faults_mask = ((static_cast<uint32_t>(1) << faults::LiveFault::LIVE_FAULTS_END) - 1) & ~persistent_faults_mask;

    public:
        FaultManager();

        uint32_t get_current_set_faults() const;
        uint32_t get_persistent_faults() const;
        ...
        bool get_current_fault(size_t fault_bit) const;
    };
